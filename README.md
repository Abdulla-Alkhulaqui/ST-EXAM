...
public class MyThread extends Thread {     volatile private int x = 3; 
    public static void main(String[] args) throws Exception {         new MyThread().foo(); 
    }     public MyThread() {         x = 10;         start(); 
    } 
    public void foo() throws Exception {         join();         x = x - 1; 
...
public class Test extends Thread {     public void run() { 
    ...
    public static void main(String[] args) {         Test test = new Test(); 
        System.out.println(test.isAlive());         test.run(); 
    }  
} 
    7. Which of these are atomic operations? A. Reading/writing reference variables 
        B. Reading/writing of all primitive types 
        C. Both 
        D. None of the above 
    8. Which statement is true of this code? 
 
public class Test extends Thread {     private static int x;     public synchronized void foo() {         int current = x;         current++;         x = current; 
    } 
    public void run() {    foo();    } 
} 
    ...
    9. Which of these statements about immutable classes is not true? A. The class should be final to avoid inheritance. 
...
public class Car { 
    ...
    public Car(int engineCapacity, int engineSerialNumber) { 
        this.engine = new Engine(engineCapacity, engineSerialNumber);     }      public int getEngineCapacity() {         return engine.getEngineCapacity(); 
    }  
...
4. Which of the following implementations of interfaces are used to store key-value pairs? A List 
    B. Set 
    C. Map 
    D. Collection 
...
3. Which statement is true for the Scrum Master? (not included)
...
4. Which statement is true for daily Scrum? (not included)
...
1. What does the DRY principle say? 
    ...
    D. It is safe to say that the quality of our code base will improve if we always leave our current code 
there a little “better”, a little cleaner than we found it. 
...
public class DamageCalculator {     private int damageFactor; 
    ...
    public int totalDamage(int baseDamage, int damageAlgorithm){         switch(damageAlgoritm) { 
            case 0: return baseDamage * damageFactor;             case 1: return baseDamage * 4;             default: return baseDamage; 
        } 
    } 
} 
...
public class Switch {     private Lamp lamp; 
     public Switch(Lamp lamp) {         this.lamp = lamp; 
    }      public void useSwitch(){         if(lamp.isOn()) {             lamp.turnOff(); 
        } else { 
            lamp.turnOn(); 
        } 
    } 
...
public MyPattern withName(String name) {     this.name = name;     return this; 
}  
public MyPattern withNumber(int number) {     this.number = number;     return this; 
} 
...
public class MyPattern {     private MyPattern(){} 
 
    private static class MyPatternInstance { 
        private static final MyPattern INSTANCE = new MyPattern(); 
    } 
    public static MyPattern getInstance() {          return MyPatternInstance.INSTANCE;  
    } 
} 
...
public abstract class Maze { 
    ...
    public Maze() { 
        Room room1 = makeRoom();         Room room2 = makeRoom();         room1.connect(room2);         rooms.add(room1);         rooms.add(room2); 	 
    } 
